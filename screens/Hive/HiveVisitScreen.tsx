...
import { updateHive, checkHiveNameExists } from "../../modules/Mock/HiveMock";
...
function HiveVisitScreen({ route, navigation }: any) {
    const insets = useSafeAreaInsets();
    const hiveInfo = route.params?.hiveInfo;
    const apiaryInfo: IApiary = route.params?.apiaryInfo;
    const apiarySettings = apiaryInfo?.settings || {};

    const [hiveData, setHiveData] = useState<IHiveData>(hiveInfo ? {
        name: hiveInfo.name || '',
        image: hiveInfo.image || '',
        status: hiveInfo.status || 'Malo',
        honey: hiveInfo.honey || 0,
        levudex: hiveInfo.levudex || 0,
        sugar: hiveInfo.sugar || 0,
        box: hiveInfo.box || 0,
        boxMedium: hiveInfo.boxMedium || 0,
        boxSmall: hiveInfo.boxSmall || 0,
        tOxalic: hiveInfo.tOxalic || 0,
        tAmitraz: hiveInfo.tAmitraz || 0,
        tFlumetrine: hiveInfo.tFlumetrine || 0,
        disease: hiveInfo.disease || '',
        production: hiveInfo.production || 0,
        queenStatus: hiveInfo.queenStatus || 'unknown',
        population: hiveInfo.population || 5,
        broodFrames: hiveInfo.broodFrames || 0,
        honeyFrames: hiveInfo.honeyFrames || 0,
        pollenFrames: hiveInfo.pollenFrames || 0,
        hiveStrength: hiveInfo.hiveStrength || 'medium',
        swarming: hiveInfo.swarming || false,
        lastInspection: hiveInfo.lastInspection || new Date().toISOString().split('T')[0],
        tComment: hiveInfo.tComment || '',
        settings: hiveInfo.settings || apiarySettings,
    } : {
        name: '',
        image: '',
        status: 'Malo',
    ...
    // Inicializar hiveStatus basado en el estado de la colmena
    ...
    const handleSubmit = async () => {
        ...
        // Validar que el código sea único (excluyendo la colmena actual)
        if (hiveInfo?.id) {
            const nameExists = await checkHiveNameExists(apiaryInfo.id, hiveData.name, hiveInfo.id);
            if (nameExists) {
                ToastAndroid.show('Ya existe otra colmena con este código en este apiario', ToastAndroid.SHORT);
                return;
            }
        }
        ...
        try {
            if (!hiveInfo?.id) {
                ToastAndroid.show('Error: No se encontró la colmena', ToastAndroid.SHORT);
                return;
            }
            
            await updateHive(hiveInfo.id, hiveData);
            ToastAndroid.show('Colmena actualizada exitosamente', ToastAndroid.SHORT);
            navigation.goBack();
        } catch (error: any) {
            ToastAndroid.show(`Error al actualizar colmena: ${error?.message || 'Error desconocido'}`, ToastAndroid.SHORT);
        } finally {
            ...
            >
                <View style={styles.container}>
                    <View style={styles.addHiveTitle}>
                        <Text style={styles.addHiveTitleText}>Visitar Colmena</Text>
                        <Text style={styles.addHiveSubTitleText}>Actualiza la información de la colmena {hiveInfo?.name || ''}</Text>
                    </View>
                </View>
...
export default HiveVisitScreen;
